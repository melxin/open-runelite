...
import net.runelite.client.plugins.openrl.external.OPRLUpdateManager;
...
public class PluginsPanel extends JPanel
{
	...
	static
	{
		final BufferedImage addIcon =
			ImageUtil.recolorImage(
				ImageUtil.loadImageResource(net.runelite.client.plugins.openrl.external.ui.PluginsPanel.class, "add_icon.png"), ColorScheme.GREEN
			);
		...
		final BufferedImage deleteImg =
			ImageUtil.recolorImage(
				ImageUtil.resizeCanvas(
					ImageUtil.loadImageResource(net.runelite.client.plugins.openrl.external.ui.PluginsPanel.class, "delete_icon.png"), 14, 14
				), ColorScheme.GREEN
			);
	...
	private final OPRLExternalPluginManager externalPluginManager;
	private final VersionManager versionManager;
	private final OPRLUpdateManager updateManager;
	...
	PluginsPanel(OPRLExternalPluginManager externalPluginManager, EventBus eventBus)
	{
		...
		final JTabbedPane mainTabPane = new JTabbedPane();
	...
	@Subscribe
	public void onOPRLRepositoryChanged(OPRLRepositoryChanged event)
	{
		buildFilter();
		reloadPlugins();
		repaint();
	...
	private List<String> getRepositories()
	{
		...
		for (OPRLUpdateRepository updateRepository : this.updateManager.getRepositories())
		{
	...
	{
		final JLabel title = new JShadowedLabel();
	...
	{
		final JPanel installedPluginsContainer = new JPanel();
	...
	{
		final JPanel availablePluginsContainer = new JPanel();
	...
	{
		final JaroWinklerDistance DISTANCE = new JaroWinklerDistance();
	...
	private void fetchPlugins()
	{
		...
		final List<String> disabledPlugins = externalPluginManager.getDisabledPluginIds();
	...
	@Subscribe
	private void onOPRLPluginChanged(OPRLPluginChanged externalPluginChanged)
	{
		final String pluginId = externalPluginChanged.getPluginId();
		...
		final ExternalBox extBox = (ExternalBox) externalBox.get();
		deps = externalPluginManager.getDependencies();
	...
	private void installedPlugins()
	{
		...
		for (PluginInfo pluginInfo : installedPluginsList)
		{
			...
			if (filterComboBox.getSelectedIndex() != 0)
			{
				...
				for (OPRLUpdateRepository updateRepository : updateManager.getRepositories())
				{
	...
	private void availablePlugins()
	{
		...
		for (PluginInfo pluginInfo : availablePluginsList)
		{
			...
			final ExternalBox pluginBox = new ExternalBox(pluginInfo);
			pluginBox.pluginInfo = pluginInfo;
					...
					else
					{
						...
						SwingWorker<Boolean, Void> worker = new SwingWorker<>()
						{
							...
							protected void done()
							{
								...
								final PluginListPanel pluginListPanel = Static.getPluginListPanelProvider().get();
