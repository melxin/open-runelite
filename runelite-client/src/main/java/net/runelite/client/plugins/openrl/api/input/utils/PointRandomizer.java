...
public class PointRandomizer
{
	...
	{
		final Shape shape = Perspective.getClickbox(Static.getClient(), actor.getWorldView(), actor.getModel(), 0, actor.getLocalLocation().getX(), actor.getLocalLocation().getY(), Static.getClient().getPlane());
		if (shape != null)
		{
			return shape.getBounds();
		}
	...
	{
		final Shape shape = object.getClickbox();
		if (shape != null)
		{
			return shape.getBounds();
		}
