...
import net.runelite.client.plugins.openrl.api.movement.unethicalite.pathfinder.model.FairyRingLocation;
import net.runelite.client.plugins.openrl.api.movement.unethicalite.pathfinder.model.GnomeGliderLocation;
...
import net.runelite.client.plugins.openrl.api.rs2.providers.quests.QuestVarPlayer;
...
import static net.runelite.client.plugins.openrl.api.movement.unethicalite.pathfinder.model.MovementConstants.DRAYNOR_MANOR_BASEMENT_DOORS;
import static net.runelite.client.plugins.openrl.api.movement.unethicalite.pathfinder.model.MovementConstants.MUSHTREES;
import static net.runelite.client.plugins.openrl.api.movement.unethicalite.pathfinder.model.MovementConstants.SLASH_ITEMS;
import static net.runelite.client.plugins.openrl.api.movement.unethicalite.pathfinder.model.MovementConstants.SLASH_WEB_POINTS;
import static net.runelite.client.plugins.openrl.api.movement.unethicalite.pathfinder.model.MovementConstants.SPIRIT_TREES;
...
public class TransportLoader
{
	...
	public static void init()
	{
		...
		try (InputStream stream = Walker.class.getResourceAsStream("/transports.json"))
		{
			...
			final TransportDto[] json = GSON.fromJson(new String(stream.readAllBytes()), TransportDto[].class);

			final List<Transport> list = Arrays.stream(json)
	...
	public static void refreshTransports()
	{
		GameThread.invoke(() ->
		{
			final List<Transport> filteredStatic = ALL_STATIC_TRANSPORTS.stream()
				.filter(it -> it.getRequirements().fulfilled())
				.collect(Collectors.toList());

			final List<Transport> transports = new ArrayList<>();

			final int gold = RS2Inventory.getFirst(995) != null ? RS2Inventory.getFirst(995).getQuantity() : 0;

			if (gold >= 30)
			{
				if (RS2Quests.isFinished(Quest.PIRATES_TREASURE))
				{
			...
			if (RS2Worlds.inMembersWorld())
			{
				...
				// Port sarim
				if (Vars.getBit(VarbitID.ZEAH_PLAYERHASVISITED) == 0) // First time talking to Veos
				{
				...
				else if (RS2Quests.isFinished(Quest.A_KINGDOM_DIVIDED)) // Veos is replaced during/after quest
				{
				...
				// Charter Ships
				if (Static.getWalkerConfig().useCharterShips())
				{
					transports.addAll(CharterShipLocation.getCharterShips(gold));
				}

				if (RS2Quests.getState(Quest.LUNAR_DIPLOMACY) != QuestState.NOT_STARTED)
				{
				...
				// Spirit Trees
				if (RS2Quests.isFinished(Quest.TREE_GNOME_VILLAGE))
				{
					for (var source : SPIRIT_TREES)
					{
						if (source.location.equals("Gnome Stronghold") && !RS2Quests.isFinished(Quest.THE_GRAND_TREE))
						{
							continue;
						}
				...
				if (RS2Quests.isFinished(Quest.THE_LOST_TRIBE))
				{
				...
				// Tree Gnome Village
				if (RS2Quests.getState(Quest.TREE_GNOME_VILLAGE) != QuestState.NOT_STARTED)
				{
				...
				// Waterbirth island
				if (RS2Quests.isFinished(Quest.THE_FREMENNIK_TRIALS) || gold >= 1000)
				{
				...
				if (RS2Quests.getState(Quest.RECIPE_FOR_DISASTER) != QuestState.IN_PROGRESS)
				{
				...
				// Fairy Rings
				if (RS2Equipment.contains(ItemID.DRAMEN_STAFF, ItemID.LUNAR_STAFF)
					&& RS2Quests.getState(Quest.FAIRYTALE_II__CURE_A_QUEEN) != QuestState.NOT_STARTED)
				{
				...
				// Gnome Gliders
				if (RS2Quests.isFinished(Quest.THE_GRAND_TREE))
				{
					for (var source : GnomeGliderLocation.values())
					{
						for (var target : GnomeGliderLocation.values())
						{
							...
							if ((source.getWorldPoint() == GnomeGliderLocation.LEMANTOLLY_UNDRI.getWorldPoint() ||
								...
								!RS2Quests.isFinished(Quest.ONE_SMALL_FAVOUR))
							{
								continue;
							}
							if ((source.getWorldPoint() == GnomeGliderLocation.OOKOOKOLLY_UNDRI.getWorldPoint() ||
								...
								!RS2Quests.isFinished(Quest.MONKEY_MADNESS_II))
							{
								continue;
							}
			...
			for (var source : MUSHTREES)
			{
				for (var target : MUSHTREES)
				{
					...
					{
						transports.add(mushtreeTransport(source.position, target.position, target.interfaceId));
					}
				}
			}
			// Tower of Life
			...
			// Port Piscarilius
			if (RS2Quests.isFinished(Quest.A_KINGDOM_DIVIDED)) // Veos is replaced during/after quest
			{
			...
			if (RS2Quests.isFinished(Quest.THE_CORSAIR_CURSE))
			{
			...
			if (RS2Inventory.contains(SLASH_ITEMS) || RS2Equipment.contains(SLASH_ITEMS))
			{
	...
	)
	{
		...
		{
			final RS2TileObject openDoor = RS2TileObjects.getFirstSurrounding(source, 1, openDoorId);
			if (openDoor != null)
			{
	...
	)
	{
		...
		{
			final RS2TileObject openedTrapdoor = RS2TileObjects.getFirstSurrounding(source, 5, openedId);
			...
			final RS2TileObject closedTrapDoor = RS2TileObjects.getFirstSurrounding(source, 5, closedId);
	...
	)
	{
		return new Transport(source.getLocation(), destination.getLocation(), Integer.MAX_VALUE, 0, () ->
		{
			...
			final RS2TileObject ring = RS2TileObjects.getFirstSurrounding(source.getLocation(), 5, "Fairy ring");
			...
			if (RS2Widgets.isVisible(RS2Widgets.get(WidgetInfo.FAIRY_RING)))
			{
				destination.travel();
				return;
			}
	...
	)
	{
		return new Transport(source, destination, Integer.MAX_VALUE, 0, () ->
		{
			final RS2Item item = RS2Inventory.getFirst(itemId);
			if (item == null)
			{
				return;
			}

			final RS2TileObject transport = RS2TileObjects.getFirstSurrounding(source, 8, objId);
			if (transport != null)
			{
	...
	)
	{
		...
		{
			final RS2NPC npc = RS2NPCs.getNearest(x -> x.getWorldLocation().distanceTo(source) <= 10 && x.getId() == npcId);
			if (npc != null)
			{
	...
	)
	{
		return new Transport(source, destination, 10, 0, () ->
		{
			final RS2NPC npc = RS2NPCs.getNearest(x -> x.getWorldLocation().distanceTo(source) <= 10 && x.getName().equalsIgnoreCase(npcName));
			if (npc != null)
			{
				npc.interact(actions);
			}
	...
	)
	{
		return new Transport(source, destination, 10, 0, () ->
		{
			...
			{
				if (RS2Dialog.chooseOption(chatOptions))
				{
					return;
				}
	...
	)
	{
		return new Transport(source, destination, Integer.MAX_VALUE, 0, () ->
		{
			final RS2TileObject first = RS2TileObjects.getFirstAt(source, objId);
			if (first != null)
			{
			...
			RS2TileObjects.getSurrounding(source, 5, x -> x.getId() == objId).stream()
	...
	)
	{
		return new Transport(source, destination, Integer.MAX_VALUE, 0, () ->
		{
			final WorldPoint localSource =
				WorldPoint.toLocalInstance(Static.getClient(), source).stream().findFirst().orElse(source);
			final RS2TileObject first = RS2TileObjects.getFirstAt(localSource, objId);
			if (first != null)
			{
			...
			RS2TileObjects.getSurrounding(localSource, 5, x -> x.getId() == objId).stream()
	...
	)
	{
		return new Transport(source, destination, Integer.MAX_VALUE, 0, () ->
		{
			...
			final RS2TileObject transport = RS2TileObjects.getFirstSurrounding(source, 5, objId);
			if (transport != null)
			{
	...
	)
	{
		return new Transport(source, destination, Integer.MAX_VALUE, 0, () ->
		{
			final RS2TileObject transport = RS2TileObjects.getFirstSurrounding(source, 5, it -> it.getName() != null && it.getName().contains("Web") && it.hasAction("Slash"));
			if (transport != null)
			{
	...
	)
	{
		return new Transport(source, destination, 10, 0, () ->
		{
			if (RS2Dialog.isViewingOptions())
			{
				RS2Dialog.chooseOption("Okay");
				return;
			}
			final RS2Widget charterWidget = RS2Widgets.get(72, 0);
			if (RS2Widgets.isVisible(charterWidget))
			{
				RS2Widget destinationWidget = RS2Widgets.get(72, destinationWidgetId);
				if (RS2Widgets.isVisible(destinationWidget))
				{
			...
			{
				final RS2NPC charterCrew = RS2NPCs.getNearest(source, "Trader Crewmember");
				if (charterCrew != null)
				{
	...
	)
	{
		return new Transport(source, destination, 10, 0, () ->
		{
			final RS2Widget gliderMap = RS2Widgets.get(138, 0);
			if (RS2Widgets.isVisible(gliderMap))
			{
				final RS2Widget destinationWidget = RS2Widgets.get(138, widgetId);
				if (RS2Widgets.isVisible(destinationWidget))
				{
					destinationWidget.interact(0);
				}
			...
			{
				final RS2NPC gnome = RS2NPCs.getNearest(source, "Captain Klemfoodle", "Captain Errdo", "Captain Dalbur",
	...
	private static Transport spritTreeTransport(WorldPoint source, WorldPoint target, String location)
	{
		return new Transport(
			...
			() ->
			{
				final RS2Widget treeWidget = RS2Widgets.get(187, 3);
				if (RS2Widgets.isVisible(treeWidget))
				{
					Arrays.stream(treeWidget.getDynamicChildren())
						.filter(child -> child.getText().toLowerCase().contains(location.toLowerCase()))
						.findFirst()
						.map(RS2Widget::new)
						.ifPresent(child -> child.interact(0));
					return;
				}

				final RS2TileObject tree = RS2TileObjects.getFirstSurrounding(source, 5, 1293, 1294, 1295);
				if (tree != null)
				{
					tree.interact(0);
				}
	...
	private static Transport mushtreeTransport(WorldPoint source, WorldPoint target, int interfaceId)
	{
		return new Transport(
			source,
			target,
			...
			() ->
			{
				final RS2Widget treeWidget = RS2Widgets.getWidget(interfaceId);
				if (RS2Widgets.isVisible(treeWidget))
				{
					treeWidget.interact(0);
					return;
				}

				final RS2TileObject tree = RS2TileObjects.getFirstSurrounding(source, 5, "Magic Mushtree");
				if (tree != null)
				{
					tree.interact("Use");
				}
	...
	public static class MagicMushtree
	{
		...
		public MagicMushtree(WorldPoint position, @Component int interfaceId)
		{
