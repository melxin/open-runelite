...
public class ExternalPluginManagerPanel extends PluginPanel
{
	...
	static
	{
		final BufferedImage addIconRaw =
			ImageUtil.loadImageResource(net.runelite.client.plugins.openrl.external.ui.ExternalPluginManagerPanel.class, "add_raw_icon.png");
		final BufferedImage addIconGh = ImageUtil
			.resizeImage(ImageUtil.loadImageResource(net.runelite.client.plugins.openrl.external.ui.ExternalPluginManagerPanel.class, "gh_icon.png"), 14, 14);
	...
	@Inject
	private ExternalPluginManagerPanel(OPRLExternalPluginManager externalPluginManager, EventBus eventBus)
	{
		super(false);
	...
	private JPanel titleBar()
	{
		...
		final JPanel buttonHolder = new JPanel(new BorderLayout());
		...
		addGHRepo.addMouseListener(new MouseAdapter()
		{
			...
			public void mousePressed(MouseEvent mouseEvent)
			{
				...
				final JTextField token = new JTextField();
				...
				if (OPRLExternalPluginManager.testGHRepository(owner.getText(), name.getText(), token.getText()))
				{
		...
		addRawRepo.addMouseListener(new MouseAdapter()
		{
			...
			public void mousePressed(MouseEvent mouseEvent)
			{
				...
				if ((pluginJson == null && OPRLExternalPluginManager.testRepository(urlActual, token.getText())) || (pluginJson != null && OPRLExternalPluginManager.testRepository(urlActual, pluginJson)))
				{
					JOptionPane.showMessageDialog(ClientUI.getFrame(), "This doesn't appear to be a valid repository.", "Error!",
						JOptionPane.ERROR_MESSAGE);
					return;
	...
	private JTabbedPane tabbedPane()
	{
		...
		final PluginsPanel pluginPanel = new PluginsPanel(this.externalPluginManager, this.eventBus);
		final JScrollPane repositoryPanel = wrapContainer(new RepositoryPanel(this.externalPluginManager, this.eventBus));
	...
	{
		final Font font = (Font) UIManager.get("OptionPane.buttonFont");
		final int answer = showWarningDialog();
	...
	{
		final Object[] options = {"Okay, I accept the risk", "Never mind, turn back"};
		final JLabel label = new JLabel("<html><p>" +
		...
		final Font font = new Font(FontManager.getRunescapeFont().getName(), FontManager.getRunescapeFont().getStyle(), 32);
		UIManager.put("OptionPane.buttonFont", font);
