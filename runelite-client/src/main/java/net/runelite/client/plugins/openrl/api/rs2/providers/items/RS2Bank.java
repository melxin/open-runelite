...
public class RS2Bank extends RS2Items
{
	...
	private static final Supplier<RS2Widget> SETTINGS_CONTAINER = () -> RS2Widgets.get(InterfaceID.BANKMAIN, 48);
	private static final Supplier<RS2Widget> WITHDRAW_ITEM = () -> RS2Widgets.getWidget(InterfaceID.Bankmain.ITEMS);
	private static final Supplier<RS2Widget> WITHDRAW_NOTE = () -> RS2Widgets.getWidget(InterfaceID.Bankmain.NOTE);
	private static final Supplier<RS2Widget> EXIT = () -> RS2Widgets.get(InterfaceID.BANKMAIN, 2, 11);
	...
	{
		return RS2ItemQuery.bankQuery();
	}

	public static RS2ItemQuery inventoryQuery()
	{
		return RS2ItemQuery.bankInventoryQuery();
	}
	...
	public static void setQuantityMode(QuantityMode quantityMode)
	{
		...
		{
			final RS2Widget component = RS2Widgets.getWidget(quantityMode.interfaceId);
	...
	{
		final RS2Widget widget = BANK_CAPACITY.get();
	...
	{
		final RS2Widget widget = RS2Widgets.getWidget(InterfaceID.Bankmain.OCCUPIEDSLOTS);
		if (RS2Widgets.isVisible(widget))
		{
			return Integer.parseInt(widget.getText());
		}
	...
	public static void releasePlaceholders()
	{
		...
		final RS2Widget widget = RELEASE_PLACEHOLDERS.get();
		if (widget != null)
		{
			widget.interact(5);
		}
	...
	{
		final RS2Widget settingsButton = RS2Widgets.getWidget(InterfaceID.Bankmain.MENU_BUTTON);
	...
	public static void deposit(Predicate<RS2Item> filter, int amount)
	{
		...
		/*final RS2Item item = BANK_INVENTORY.getFirst(filter);
		if (item == null)
		{
			return;
		}
	...
	{
		withdrawAll(Predicates.idEquals(id), withdrawMode);
	}

	public static void withdrawAll(String name, WithdrawMode withdrawMode)
	{
		withdrawAll(Predicates.nameEquals(name), withdrawMode);
	}
	...
	public static void withdraw(Predicate<RS2Item> filter, int amount, WithdrawMode withdrawMode)
	{
		...
		final String action = getAction(item, amount, true);
		item.interact(action);
	...
	{
		withdrawLastQuantity(Predicates.idEquals(id), withdrawMode);
	}

	public static void withdrawLastQuantity(String name, WithdrawMode withdrawMode)
	{
		withdrawLastQuantity(Predicates.nameEquals(name), withdrawMode);
	}
	...
	public static void withdrawLastQuantity(Predicate<RS2Item> filter, WithdrawMode withdrawMode)
	{
		...
		final WithdrawOption withdrawOption = WithdrawOption.LAST_QUANTITY;
		if (withdrawMode == WithdrawMode.NOTED && !isNotedWithdrawMode())
		{
			setWithdrawMode(true);
		}
	...
	{
		final RS2Widget widget = noted ? WITHDRAW_NOTE.get() : WITHDRAW_ITEM.get();
		if (widget != null)
		{
			widget.interact(0);
		}
	...
	{
		return RS2Widgets.getChildren(TAB_CONTAINER.get(), x -> x.hasAction("Collapse tab"));
	}
	...
	public static void collapseTabs()
	{
		...
		{
			final RS2Widget tab = getTabs().get(i);
			if (tab != null)
			{
				tab.interact("Collapse tab");
			}
	...
	public static void collapseTab(int index)
	{
		...
		final RS2Widget tabContainer = TAB_CONTAINER.get();

		if (RS2Widgets.isVisible(tabContainer) && isTabOpen(index))
		{
			tabContainer.getChild(10 + index).interact("Collapse tab");
		}
	...
	/*private static int getActionIndex(RS2Item item, int amount, boolean withdraw)
	{
		if (amount == 1)
		{
			return 2;
		...
		else if (amount == 10)
		{
			...
			|| withdraw && amount >= item.getQuantity()
			|| !withdraw && amount >= BANK_INVENTORY.getCount(true, item.getId()))
		{
			return 8;
		}
		else
	...
	{
		final RS2Widget exitBank = EXIT.get();
	...
	{
		final Rectangle containerBounds = RS2Widgets.getWidget(InterfaceID.Bankmain.ITEMS).getBounds();
		final Rectangle itemBounds = item.getWidgetItem().getWidget().getBounds();

		return containerBounds.contains(itemBounds);
	}
	...
	public static void scrollToItem(RS2Item item)
	{
		...
		while (!isInViewport(item))
		{
			...
			if (itemBounds.y + itemBounds.height < containerBounds.y)
			{
				Mouse.scrollUp(new Point(containerBounds.x + containerBounds.width / 2, containerBounds.y + containerBounds.height / 2));
			}
			else if (itemBounds.y > containerBounds.y + containerBounds.height)
			{
				Mouse.scrollDown(new Point(containerBounds.x + containerBounds.width / 2, containerBounds.y + containerBounds.height / 2));
			}
			else
			{
				break;
