...
public class RS2MiniMap
{
	...
	{
		return localPoint == null ? null : Static.getClientThread().runOnClientThreadResult(() -> Perspective.localToMinimap(Static.getClient(), localPoint));
	}
	...
	public static Point worldToMinimap(WorldPoint worldPoint)
	{
		...
		return Static.getClientThread().runOnClientThreadResult(() -> Perspective.localToMinimap(Static.getClient(), lp));
	}
	...
	public static Shape getMinimapClipArea(double scale)
	{
		...
		final BufferedImage minimapSprite = Static.getClientThread().runOnClientThreadResult(() ->
	...
	/**
	 * Draws a world point path on the minimap.
	 *
	 * @param graphics the graphics context to draw on.
	 * @param path the world points to draw.
	...
	public static void drawPath(Graphics2D graphics, List<WorldPoint> path, Color color)
	{
		...
		{
			drawWorldPointNode(graphics, point, color);
		}
	}

	/**
	 * Draws a world point on the minimap.
	 *
	 * @param graphics the graphics context to draw on.
	 * @param point the world points to draw.
	 * @param color the color of the drawing.
	 */
	public static void drawWorldPointNode(Graphics2D graphics, WorldPoint point, Color color)
	{
		if (point.distanceTo(RS2Players.getLocal().getWorldLocation()) >= 16)
		{
			return;
		}
		...
		final net.runelite.api.Point posOnMinimap = Perspective.localToMinimap(client, lp);
