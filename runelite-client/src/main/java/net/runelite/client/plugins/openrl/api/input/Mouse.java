...
public class Mouse
{
	...
	public static void click(Rectangle point, boolean rightClick)
	{
	...
	private static void handleClick(int x, int y, boolean rightClick)
	{
		...
		moved(x + (int) (Math.random() * 2 - 1), y + (int) (Math.random() * 2 - 1), canvas, System.currentTimeMillis());
		Time.sleep(8, 60);
	...
	public static void drag(Rectangle startPoint, Rectangle endPoint, boolean rightClick)
	{
	...
	private static void handleDrag(int startX, int startY, int endX, int endY, boolean rightClick)
	{
		...
		exited(startX, startY, canvas, System.currentTimeMillis());
		Static.getNaturalMouse().moveTo(startX, endY);
		moved(startX + (int) (Math.random() * 2 - 1), startY + (int) (Math.random() * 2 - 1), canvas, System.currentTimeMillis());
		Time.sleep(8, 60);
		...
		moved(endX + (int) (Math.random() * 2 - 1), endY + (int) (Math.random() * 2 - 1), canvas, System.currentTimeMillis());
		Time.sleep(600, 800);
	...
	public static void scrollUp(Point point)
	{
		...
		moved((int) point.getX(), (int) point.getY(), canvas, System.currentTimeMillis());

		final MouseEvent event = new MouseWheelEvent(canvas, MouseEvent.MOUSE_WHEEL, System.currentTimeMillis(), 0, (int) point.getX(), (int) point.getY(), 0, false, 0, -10, -2);
		event.setSource("openrl");
		canvas.dispatchEvent(event);
	}
