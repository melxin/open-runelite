...
public class RS2Worlds
{
	...
	public static List<World> getAll(Predicate<World> filter)
	{
		...
		try
		{
			final World[] worlds = Static.getClient().getWorldList();
			if (worlds == null)
			{
				loadWorlds();
				return out;
	...
	{
		final List<World> all = getAll(filter);
		if (all.isEmpty())
		{
			return null;
		}
	...
	public static void hopTo(World world, boolean spam)
	{
		...
		final RS2Widget rememberOption = RS2Dialog.getOptions().stream()
		...
		Static.invokeMenuAction("Switch", "<col=ff9040>" + (world.getId()) + "</col>", 1, MenuAction.CC_OP.getId(), world.getId(), InterfaceID.Worldswitcher.BUTTONS, -1, -1, -1);
		if (!spam)
		{
			Time.sleepUntil(() -> Game.getState() == GameState.HOPPING, 3000);
		}
		...
		{
			RS2Dialog.chooseOption("Switch world");
			Time.sleepUntil(() -> Game.getState() == GameState.HOPPING, 3000);
		}
	}
	...
	public static boolean inMembersWorld(boolean useLookUp)
	{
		if (useLookUp)
		{
			final World currentWorld = lookup().stream()
	...
	public static void openHopper()
	{
		...
		Static.invokeMenuAction("World Switcher", "", 1, MenuAction.CC_OP.getId(), -1, InterfaceID.Logout.WORLD_SWITCHER, -1, -1, -1);
	}
	...
	{
		return RS2Widgets.isVisible(RS2Widgets.getWidget(InterfaceID.Worldswitcher.LIST));
	}
