package net.runelite.client.plugins.openrl.api.rs2.providers.camera;
...
public class RS2Camera
{
	...
	public static int angleToTile(RS2Actor t)
	{
		int angle = (int) Math.toDegrees(Math.atan2(t.getWorldLocation().getY() - RS2Players.getLocal().getWorldLocation().getY(),
			t.getWorldLocation().getX() - RS2Players.getLocal().getWorldLocation().getX()));
		return angle >= 0 ? angle : 360 + angle;
	}

	public static int angleToTile(RS2TileObject t)
	{
		int angle = (int) Math.toDegrees(Math.atan2(t.getWorldLocation().getY() - RS2Players.getLocal().getWorldLocation().getY(),
			t.getWorldLocation().getX() - RS2Players.getLocal().getWorldLocation().getX()));
		return angle >= 0 ? angle : 360 + angle;
	...
	{
		int angle = (int) Math.toDegrees(Math.atan2(localPoint.getY() - RS2Players.getLocal().getLocalLocation().getY(),
			localPoint.getX() - RS2Players.getLocal().getLocalLocation().getX()));
		return angle >= 0 ? angle : 360 + angle;
	}
	...
	public static void turnTo(final RS2Actor actor)
	{
		int angle = getCharacterAngle(actor);
		setAngle(angle, 40);
	}

	public static void turnTo(final RS2Actor actor, int maxAngle)
	{
		int angle = getCharacterAngle(actor);
		setAngle(angle, maxAngle);
	}

	public static void turnTo(final RS2TileObject tileObject)
	{
		int angle = getObjectAngle(tileObject);
		setAngle(angle, 40);
	}

	public static void turnTo(final RS2TileObject tileObject, int maxAngle)
	{
		int angle = getObjectAngle(tileObject);
		setAngle(angle, maxAngle);
	}
	...
	public static int getCharacterAngle(RS2Actor actor)
	{
		return getTileAngle(actor);
	}

	public static int getObjectAngle(RS2TileObject tileObject)
	{
		return getTileAngle(tileObject);
	}

	public static int getTileAngle(RS2Actor actor)
	{
		int a = (angleToTile(actor) - 90) % 360;
		return a < 0 ? a + 360 : a;
	}

	public static int getTileAngle(RS2TileObject tileObject)
	{
	...
	public static boolean isTileOnScreen(RS2TileObject tileObject)
	{
	...
	{
		RS2ClientScript.runScript(ScriptID.CAMERA_DO_ZOOM, zoom, zoom);
	}
	...
	public static class NpcTracker
	{
		...
		private static void trackNpc(int npcId)
		{
			...
			RS2Actor actor = (RS2Actor) RS2NPCs.getNearest(npcId).getNpc(); // Get the actor
			if (actor == null)
			{
	...
	public static boolean isInClickableViewport(SceneEntity sceneEntity)
	{
		if (sceneEntity == null)
		{
			return false;
		...
		if (sceneEntity instanceof RS2Actor)
		{
			final Polygon canvasTilePoly = ((RS2Actor) sceneEntity).getCanvasTilePoly();
			return canvasTilePoly != null ? clickableViewport.intersects(canvasTilePoly.getBounds()) : false;
		}
		else if (sceneEntity instanceof RS2TileObject)
		{
			final Polygon canvasTilePoly = ((RS2TileObject) sceneEntity).getCanvasTilePoly();
			return canvasTilePoly != null ? clickableViewport.intersects(canvasTilePoly.getBounds()) : false;
		}
		else if (sceneEntity instanceof RS2TileItem)
		{
			final Polygon canvasTilePoly = ((RS2TileItem) sceneEntity).getCanvasTilePoly();
			return canvasTilePoly != null ? clickableViewport.intersects(canvasTilePoly.getBounds()) : false;
		}
		return false;
	}

	public static void turnToSceneEntityIfOutsideClickableViewport(SceneEntity sceneEntity)
	{
		...
		if (sceneEntity instanceof RS2Actor)
		{
			turnTo(((RS2Actor) sceneEntity));
		}
		else if (sceneEntity instanceof RS2TileObject)
		{
			turnTo(((RS2TileObject) sceneEntity));
		}
		else if (sceneEntity instanceof RS2TileItem)
		{
