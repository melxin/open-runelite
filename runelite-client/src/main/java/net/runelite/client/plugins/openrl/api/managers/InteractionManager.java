...
public class InteractionManager
{
	...
	private static final boolean useInvokes = true;
	...
	private void interact(int param0, int param1, MenuAction menuAction, int index, int itemId, int worldViewId, String option, String target, int canvasX, int canvasY)
	{
		...
		{
			interactMouse(canvasX, canvasY, 1);
		}
	}

	public void interactMouse(int x, int y, int button)
	{
		/*final Rectangle bounds = PointRandomizer.getBoundsFor(npc);
		int x = (int) bounds.getX();
		int y = (int) bounds.getY();*/

		naturalMouse.moveTo(x, y);

		/*final MouseHandler mouseHandler = new MouseHandler();
		mouseHandler.sendMovementMouseEvent(x, y);
		log.info("Sending click to [{}, {}]", x, y);
		mouseHandler.sendClickMouseEvent(x, y, 1);
		Time.sleep(50, 100);*/
