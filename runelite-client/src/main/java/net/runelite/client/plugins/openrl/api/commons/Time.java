...
public class Time
{
	private static final Logger logger = LoggerFactory.getLogger(net.runelite.client.plugins.openrl.api.commons.Time.class);
	...
	public static boolean sleepTicks(int ticks)
	{
		...
		if (Static.getClient().getGameState() == GameState.LOGIN_SCREEN || Static.getClient().getGameState() == GameState.LOGIN_SCREEN_AUTHENTICATOR)
		{
			return false;
		}
		...
		{
			final long start = Static.getClient().getTickCount();
	...
	{
		final long secs = Math.abs(duration.getSeconds());
