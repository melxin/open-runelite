...
public class Predicates
{
	...
	public static <T extends Identifiable> Predicate<T> idEquals(int id)
	{
		return t -> t.getId() == id;
	}

	public static <T extends Identifiable> Predicate<T> idEquals(int... ids)
	{
		return t ->
		{
			final int identifiableId = t.getId();
			for (int id : ids)
			{
				if (identifiableId == id)
				{
					return true;
				}
			}
	...
	public static <T extends Identifiable> Predicate<T> idEquals(Collection<Integer> ids)
	{
		return t -> ids.contains(t.getId());
	}

	public static <T extends EntityNameable> Predicate<T> nameEquals(String name)
	{
		return t ->
		{
			final String entityName = t.getName();
			return entityName != null && name != null && Text.sanitize(entityName).equalsIgnoreCase(name);
		};
	}

	public static <T extends EntityNameable> Predicate<T> nameEquals(String... names)
	{
		return t ->
		{
			final String entityName = t.getName();
			...
			{
				if (name != null && Text.sanitize(entityName).equalsIgnoreCase(name))
				{
					return true;
				}
			}
	...
	public static <T extends EntityNameable> Predicate<T> nameEquals(Collection<String> names)
	{
		return t ->
		{
			final String entityName = t.getName();
			return entityName != null && names.contains(Text.sanitize(entityName));
		};
	}

	public static <T extends EntityNameable> Predicate<T> nameContains(String name)
	{
		return t ->
		{
			final String entityName = t.getName();
			return entityName != null && name != null && Text.sanitize(entityName).toLowerCase().contains(name.toLowerCase());
		};
	}

	public static <T extends EntityNameable> Predicate<T> nameContains(String... names)
	{
		return t ->
		{
			final String entityName = t.getName();
			...
			for (String name : names)
			{
				if (name != null && Text.sanitize(entityName).toLowerCase().contains(name.toLowerCase()))
				{
					return true;
				}
			}
	...
	public static <T extends Interactable> Predicate<T> actionEquals(String action)
	{
		return t ->
		{
			final String[] interactableActions = t.getActions();
			if (interactableActions == null)
			{
				return false;
			}
			for (String interactableAction : interactableActions)
			{
				if (interactableAction != null && Text.removeTags(interactableAction).equalsIgnoreCase(action))
				{
					return true;
				}
			}
	...
	public static <T extends Interactable> Predicate<T> actionEquals(String... actions)
	{
		return t ->
		{
			final String[] interactableActions = t.getActions();
	...
	public static <T extends Interactable> Predicate<T> actionContains(String action)
	{
		return t ->
		{
			final String[] interactableActions = t.getActions();
			...
			for (String interactableAction : interactableActions)
			{
				if (interactableAction != null && Text.removeTags(interactableAction).toLowerCase().contains(action.toLowerCase()))
				{
					return true;
				}
			}
	...
	public static <T extends Interactable> Predicate<T> actionContains(String... actions)
	{
		return t ->
		{
			final String[] interactableActions = t.getActions();
	...
	/**
	 * Text equals (ignore case).
	 *
	 * @param text
	 * @return {@code true} if text equals (ignore case), {@code false} otherwise.
	 */
	public static Predicate<String> textEquals(String text)
	{
		return t -> t != null && t.equalsIgnoreCase(text);
	}
	 ...
	 * @return {@code true} if text equals (ignore case), {@code false} otherwise.
	 */
	public static Predicate<String> textEquals(String... texts)
	{
		return t ->
		{
			if (t == null)
	...
	/**
	 * Text contains (ignore case).
	 *
	 * @param texts
	 * @return {@code true} if text contains (ignore case), {@code false} otherwise.
	 */
	public static Predicate<String> textContains(String... texts)
	{
		return t ->
		{
			if (t == null)
	...
	/**
	 * Text matching predicate filter.
	 *
	 * @param predicate the predicate filter
	 * @return {@code true} if predicate filter matches, {@code false} otherwise.
	 */
	public static Predicate<String> text(Predicate<String> predicate)
	{
