...
public class RS2TileItems
{
	...
	{
		return TILE_ITEMS.all(x -> Arrays.asList(names).contains(x.getName()));
	}
	...
	{
		return getNearest(to, x -> Arrays.asList(names).contains(x.getName()));
	}
	...
	{
		return TILE_ITEMS.at(tile, x -> Arrays.asList(names).contains(x.getName()));
	}
