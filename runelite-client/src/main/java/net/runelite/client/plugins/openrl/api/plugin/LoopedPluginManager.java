...
public class LoopedPluginManager
{
	...
	{
		final Plugin plugin = pluginChanged.getPlugin();
		if (pluginChanged.isLoaded())
		{
			if (plugin instanceof SettingsPlugin)
			{
				final SettingsPlugin settingsPlugin = (SettingsPlugin) plugin;
				final PluginListPanel pluginListPanel = pluginListPanelProvider.get();
