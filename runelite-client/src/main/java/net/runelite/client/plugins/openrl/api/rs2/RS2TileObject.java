...
public class RS2TileObject implements TileObject
{
	...
	{
		return Static.getClientThread().runOnClientThreadOptional(() -> Static.getClient().getObjectDefinition(tileObject.getId())).orElse(null);
	}
	...
	public void interact(TileObject tileObject, int index)
	{
		if (index == -1)
		{
			return;
	...
	{
		interact(tileObject, getActionIndex(action));
	}
