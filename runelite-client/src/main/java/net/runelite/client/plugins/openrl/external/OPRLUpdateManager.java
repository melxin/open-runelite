...
public class OPRLUpdateManager
{
	...
	protected List<OPRLUpdateRepository> repositories;

	public OPRLUpdateManager(PluginManager pluginManager)
	{
	...
	public OPRLUpdateManager(PluginManager pluginManager, List<OPRLUpdateRepository> repos)
	{
		this(pluginManager);
	...
	{
		final List<PluginInfo> availablePlugins = new ArrayList<>();
	...
	{
		final List<PluginInfo> updates = new ArrayList<>();
	...
	{
		final List<PluginInfo> list = new ArrayList<>(getPluginsMap().values());
	...
	{
		final Map<String, PluginInfo> pluginsMap = new HashMap<>();
		for (OPRLUpdateRepository repository : getRepositories())
		{
	...
	public List<OPRLUpdateRepository> getRepositories()
	{
	...
	{
		for (OPRLUpdateRepository ur : repositories)
		{
		...
		repositories.add(new OPRLUpdateRepository(id, url));
	}
	 ...
	 */
	public void addRepository(OPRLUpdateRepository newRepo)
	{
		for (OPRLUpdateRepository ur : repositories)
		{
	...
	{
		for (OPRLUpdateRepository repo : getRepositories())
		{
	...
	public synchronized void refresh()
	{
		...
		for (OPRLUpdateRepository updateRepository : repositories)
		{
	...
	public synchronized boolean installPlugin(String id, String version)
	{
		...
		final Path downloaded = downloadPlugin(id, version);

		final Path pluginsRoot = pluginManager.getPluginsRoot();
		final Path file = pluginsRoot.resolve(downloaded.getFileName());
		try
		{
		...
		final String pluginId = pluginManager.loadPlugin(file);
		final PluginState state = pluginManager.startPlugin(pluginId);
	...
	protected Path downloadPlugin(String id, String version)
	{
		try
		{
			final PluginInfo.PluginRelease release = findReleaseForPlugin(id, version);
			final Path downloaded = getFileDownloader(id).downloadFile(new URL(release.url));
	...
	{
		final PluginInfo pluginInfo = getPluginsMap().get(id);
		if (pluginInfo == null)
		{
	...
	public boolean updatePlugin(String id, String version)
	{
		...
		final String newPluginId = pluginManager.loadPlugin(file);
		final PluginState state = pluginManager.startPlugin(newPluginId);
	...
	public boolean hasPluginUpdate(String id)
	{
		...
		final String installedVersion = pluginManager.getPlugin(id).getDescriptor().getVersion();
		final PluginInfo.PluginRelease last = getLastPluginRelease(id);
	...
	protected synchronized void initRepositoriesFromJson()
	{
		...
		try (FileReader reader = new FileReader(repositoriesJson.toFile()))
		{
			...
			final OPRLUpdateRepository[] items = gson.fromJson(reader, OPRLUpdateRepository[].class);
