...
import static net.runelite.client.plugins.openrl.api.movement.unethicalite.pathfinder.TransportLoader.charterTransport;
...
public enum CharterShipLocation
{
	...
	static
	{
		...
		if (RS2Quests.getState(Quest.CABIN_FEVER) == QuestState.FINISHED)
		{
			...
			if (RS2Quests.getState(Quest.REGICIDE) == QuestState.FINISHED)
			{
				priceMap.put(Pair.of(MOS_LE_HARMLESS_DOCK.getWorldPoint(), PORT_TYRAS_SHIP.getWorldPoint()), 3200);
			}
			if (RS2Quests.getState(Quest.SONG_OF_THE_ELVES) == QuestState.FINISHED)
			{
	...
	public static Collection<Transport> getCharterShips(int gold)
	{
		...
		double priceModifier = 1.0;
		if (RS2Equipment.contains(ItemID.RING_OF_CHAROSA))
		{
			priceModifier *= 0.5;
		}
		if (RS2Quests.getState(Quest.CABIN_FEVER) == QuestState.FINISHED)
		{
			...
			if (gold >= 3200 * priceModifier && RS2Quests.getState(Quest.REGICIDE) == QuestState.FINISHED)
			{
		...
		if (RS2Quests.getState(Quest.SONG_OF_THE_ELVES) == QuestState.FINISHED)
		{
			...
			if (gold >= 4950 * priceModifier && RS2Quests.getState(Quest.CABIN_FEVER) == QuestState.FINISHED)
			{
